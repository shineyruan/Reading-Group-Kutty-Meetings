...
\begin{equation}\label{eq:maximum_likelihood}
    \nabla \bigg(\left(\prod_{n=1}^{N}h(\mathbf{x}_n)\right)g(\bm{\eta})^N\exp\left\{\bm{\eta}^T\sum_{n=1}^N\mathbf{u}(\mathbf{x}_n)\right\}\bigg) =0
\end{equation}
...
\end{displaymath} in the form of $p(\mathbf{x}|\bm{\eta})=\left(\prod_{n=1}^{N}h(\mathbf{x}_n)\right)g(\bm{\eta})^{N}\exp\left\{\bm{\eta}^T\sum_{n=1}^{N}\mathbf{u}(\mathbf{x}_n)\right\}$, and the prior
