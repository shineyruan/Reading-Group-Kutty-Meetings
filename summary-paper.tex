...
Suppose each agent would have observed $m$ datapoints before his or her entrance to the market. He would update his belief by % equation in Helen's part 
$$\nu\leftarrow\nu+m$$, that is, he would update the first entry of the vector as the sum of data he is now having access to, the second entry as the number of these data. He would also derive the amount of outstanding shares $\theta$ from the current market price, in our case, $\theta = -1/p$ ($p$ is the current market price).
