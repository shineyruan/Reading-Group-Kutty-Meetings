...
For the setup, there would be a group of Bayesian agents (the number of agents as hyper-parameter would be an input for the program), a market maker and an executable program driving the market. They are explained in Sec~\ref{Bayesian-agents}, Sec~\ref{trading-procedure} and Sec~\ref{main-executable}. The dataset the market is based on is subject to Bernoulli distribution.

\subsection{Bayesian Agents}\label{Bayesian-agents}
Each Bayesian agents would be initialized with several data points, based on which he would form his prior $p(\boldsymbol{\eta}; \boldsymbol{\chi}, \nu)$. The dataset is subject to Bernoulli distribution, \textbf{in which $\nu$ would be the number of trades the market maker publishes, $\boldsymbol{\chi}$ would be the current market price.}
...
$$
    \delta=\ln{\left( \frac{\nu m \chi+\sum_{n=1}^{m}x_n}{\nu m+m-(\nu m \chi+\sum_{n=1}^{m}x_n)} \right)}-\eta
$$
...
\begin{enumerate}
    \item The market maker sets an initial market price $\chi=\chi_0$, and sets the number of trades $\nu=0$.
    ...
    \item He sees the current market price $\chi$ and the number of trades $\nu$. He then knows that $\nu$ agents have traded in the market before him. Under the assumption that all agents observe the same number of data points, the total number of initial pseudo observations would be $\nu m$ with an average of $\chi$.
...
As discussed in Sec~\ref{Bayesian-agents} and Sec~\ref{trading-procedure}, each agent would update their belief and then trade with market maker. The final market price is considered as the aggregated belief among agents.
